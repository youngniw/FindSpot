...
public class SelectWhomActivity extends AppCompatActivity {
    ...
    void selwhom_radiogroup_checkedChangeListener() {
        rg_selwhom_radiogroup.setOnCheckedChangeListener((group, checkedId) -> {
    ...
    void selwhom_btn_clickListener() {
        btn_selwhom_next.setOnClickListener(v -> {
            ...
            if (isRandom) {
                ...
                try {
                    new GetGroupInfoTask().execute().get();      //DB로부터 그룹의 속한 사용자들의 닉네임, 도로명, 지정한 x, y값을 받아옴, 또한 최근에 찾은 위치를 불러옴
